...
from ..config import BoolField, NumberField, PathField, StringField
...
class AmazonProductData(BaseFormatConverter):
    ...
    def convert(self, check_content=False, **kwargs):
        ...
        input_folder = preprocessed_folder / f"bs{self.batch}" / "input"
        ...
        for src, tgt in test_data:
            ...
            c_input = input_folder / f"{subfolder:02d}"
            c_input = c_input / f"{iteration:06d}.npz"
            ...
            identifiers = [
                f"{self.mid_his_batch}_{self.separator}{c_file}",
                f"{self.cat_his_batch}_{self.separator}{c_file}",
                f"{self.uid_batch}_{self.separator}{c_file}",
                f"{self.mid_batch}_{self.separator}{c_file}",
                f"{self.cat_batch}_{self.separator}{c_file}",
                f"{self.mask}_{self.separator}{c_file}",
                f"{self.seq_len}_{self.separator}{c_file}",
            ]
